/**
 * Project name - comics
 * File name - CharPicController.js
 * Description - (Characters pictures) Controller handling request
 */
class CharPicController {
    constructor({ comicService }) {
        this.service = comicService;
    }
     ...
     */
     read = async (req, reply) => {
        try {
            let data = await this.service.readChars.execute(req.query);
            reply.status(200).send(data);
        } catch (error) {
            reply.errorHandler(error);
        }
...
module.exports = CharPicController;
